...
CommonParams ReadCommonParams(istream& inputStream) {
    CommonParams params;
    inputStream >> params.machineType >> params.machineTypeOut >> params.stateNumber >> params.inputSymbolNumber >> params.outputSignalNumber;
    return params;
}
...
void WriteMiliResultMatrix(ostream& output, vector<vector<MachineState>> startMatrix)
{
    for (int i = 0; i < startMatrix.size(); i++)
    {
        for (int j = 0; j < startMatrix.size(); j++)
        {
            output << startMatrix[i][j].outputState << "\t";
        }
        output << endl;
        for (int j = 0; j < startMatrix.size(); j++)
        {
            output << startMatrix[i][j].outputSignal << "\t";
        }
        output << endl << endl;
    }
}

void WriteMooreResultMatrix(ostream& output, vector<vector<MachineState>> startMatrix)
{
    for (int j = 0; j < startMatrix.size(); j++)
    {
        output << startMatrix[0][j].outputSignal << "\t";
    }
    output << endl;
...
void WriteResultMatrix(ostream& output, vector<vector<MachineState>> startMatrix, string machineType)
{
    if (machineType == MEALY_MACHINE_TYPE)
    {
        WriteMiliResultMatrix(output, startMatrix);
        return;
    }
    WriteMooreResultMatrix(output, startMatrix);
}
...
int main(int argc, char* argv[])
{
    ...
    WriteResultMatrix(outputFile, inputMatrix, commonParams.machineTypeOut);
}